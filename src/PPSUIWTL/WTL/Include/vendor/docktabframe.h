...
namespace DockSplitTab {
		...
		public:
			...
			LRESULT OnPaint( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/) {
				...
				if ( !rect.IsRectEmpty()) {
					...
					UINT uFormat = DT_INTERNAL | DT_LEFT | DT_MODIFYSTRING | DT_END_ELLIPSIS | DT_NOPREFIX;
#ifndef _WIN32_WCE
			...
			if ( NULL != position)
				do {
					...
					if ( clientView->owner == splitPane)
						...
						case stateNull:     stateSet.null     |= 1; break;
						case stateDockable: stateSet.dockable |= 1; break;
						case stateFloating: stateSet.floating |= 1; break;
						}
				} while ( NULL != position);
			return stateSet;
		}
	...
	public:
		...
		bool toggleClientViewState( HWND clientViewWnd, State state) {
		...
		bool dockClientView( DockSide dockSide, ClientView* clientView) {
			...
			this->clientViews[ clientView->wnd] = clientView;
			clientView->owner = this->currentPane;
			clientView->state = stateDockable;
			return this->currentPane->append( clientView->caption
			                                , clientView->wnd
			                                , clientView->toolTip
			                                , clientView->imageIndex
		...
		HWND floatClientView( LPRECT rect, ClientView* clientView) {
			...
			clientView->state = stateFloating;
			return floatFrame->m_hWnd;
		}
